...
const MLIR_LIBS = [_][]const u8{
    ...
    // Required by code for lowering MLIR to LLVMIR
    ...
    // Required by the function "mlir::makeOptimizingTransformer()"
    // (defined in "mlir/lib/ExecutionEngine/OptUtils.cpp")
...
const MLIR_CAPI_LIBS = [_][]const u8{
    ...
    // XXX: if we don't want to use "RegisterEverything" for loading all available
    // dialects and extensions, we have to figure out what else libraries need
    // to link against.
    // "LLVM",
};
...
const LLVM_LIBS = [_][]const u8{
    ...
    // Required by "llvm-c/TargetMachine.h"
    "Target",
};
...
const MLIR_CONVERSION_LIBS = [_][]const u8{
    "AffineToStandard",
    "AMDGPUToROCDL",
...
// related to "nativecodegen" when setting `LLVM_LINK_COMPONENTS` in cmake
const LLVM_TARGET_LIBS = [_][]const u8{
    "X86CodeGen",
    "X86AsmParser",
    "X86Desc",
    "X86Info",
...
pub fn build(
    ...
    bc.linkLibs(exe, "MLIR", &MLIR_CONVERSION_LIBS);
    bc.linkLibs(exe, "LLVM", &LLVM_TARGET_LIBS);
    return exe;
}
