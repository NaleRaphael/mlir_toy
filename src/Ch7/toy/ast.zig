...
pub const VarDeclExprASTListType = ListType(*VarDeclExprAST);
...
pub const RecordASTListType = ListType(RecordAST);
...
test "struct literal expr" {
    ...
    const _lit_val_2 = _lit_vals[1].Num;
...
test "call expr" {
    ...
    const fname = "foobar.toy";
    var arg_1 = try NumberExprAST.init(test_alloc, newLoc(fname, 2, 9), 11);
    var arg_2 = try NumberExprAST.init(test_alloc, newLoc(fname, 2, 13), 13);
    var args_al = ExprASTListType.ArrayList.init(test_alloc);
...
test "struct ast" {
    ...
    const _struct_vec2 = type_erased_vec2.Struct;
    ...
    const _struct_foo = type_erased_foo.Struct;
